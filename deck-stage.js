  ...
  class DeckStage extends HTMLElement {
    ...
    _applyIndex({ showOverlay = true, broadcast = true, reason = 'init' } = {}) {
      ...
      this._slides.forEach((s, i) => {
        if (s._deckFadeTimer) {
          clearTimeout(s._deckFadeTimer);
          s._deckFadeTimer = null;
        }
        s.removeAttribute('data-deck-fading');
        if (i === curr) {
          s.setAttribute('data-deck-active', '');
        } else {
          if (i === prev && prev !== curr && reason !== 'init') {
            s.setAttribute('data-deck-fading', '');
            s._deckFadeTimer = setTimeout(() => {
              s.removeAttribute('data-deck-fading');
              s._deckFadeTimer = null;
            }, 200);
          }
